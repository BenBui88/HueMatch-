...
function LandingPage() {
  ...
  const handleRoleSelect = (role: UserRole) => {
    setSelected(role); setShowForm(true); setError(''); setIsSignUp(false)
  }
  ...
  const signIn = async () => {
    ...
    try {
      if (isSignUp) {
        ...
        if (e) { setError('Invalid email or password. Try signing up instead.'); setLoading(false); return }
      }
      ...
      {showForm && (
        <>
          ...
          <div style={{ position:'fixed', top:'5%', left:'50%', transform:'translateX(-50%)', width:'90%', maxWidth:390, background:'white', borderRadius:20, padding:'1.25rem 1.5rem 1.5rem', zIndex:50, boxShadow:'0 4px 24px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ textAlign:'center', fontSize:12, color:'#6b6b80', marginTop:12, margin:'12px 0 0' }}>
...
function MatchScreen() {
  ...
  const selectSkin = (t: typeof SKIN_TONES[0]) => {
    setSkinHexState(t.hex);setSkinNameState(t.name);setUndertoneState(t.ut);setSkin(t.hex,t.name,t.ut)
  }
          ...
          <div style={{border:skinHex?'1.5px solid #C4546A':'0.5px solid #ddd',borderRadius:12,overflow:'hidden',padding:8}}>
            ...
            {skinHex && (
              <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6,padding:'4px 6px',background:'#f8f7f9',borderRadius:7}}>
                ...
                <button onClick={()=>{setSkinHexState('');setSkinNameState('');setSkin('','','')}} style={{fontSize:9,color:'#C4546A',border:'none',background:'none',cursor:'pointer',flexShrink:0}}>✕</button>
              </div>
            )}
